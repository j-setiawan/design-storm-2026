// Team addition (teams/team-tbd): the chat window, shared by design-storm-water-system-3d.html
// and strontia.html. POSTs to /api/chat, which only exists under teams/team-tbd/chat_server.py
  ...
  async function chatSend(text) {
    ...
    try {
      ...
      if (!r.ok) throw new Error(r.status === 501 || r.status === 404 || r.status === 405
        ? "Chat needs the team server. Stop serve.py, run `python3 teams/team-tbd/chat_server.py` from the repo root instead (same port, same pages), and reload."
